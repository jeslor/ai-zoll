# AGENTS.md

Instructions for AI coding agents working in this project.

## Project

School Management Platform — A platform for schools to manage students, teachers and payments.

See PROJECT.md for the full project overview.

## Architecture

This project follows Modular Architecture. See ARCHITECTURE.md for details.

## Stack

- Frontend: nextjs
- Backend: nestjs
- Database: postgresql (via prisma)

Do not introduce a different framework or library for any of these roles without
updating the Blueprint first.

## Testing requirements

New functionality must include unit tests, integration tests, end-to-end (e2e) tests.

## Security requirements

- Authentication: jwt
- Authorization: rbac

Follow these mechanisms consistently; do not introduce an alternative
authentication/authorization approach without updating the Blueprint first.

---
Generated by AI Software Zoll from Blueprint version 1.0.
